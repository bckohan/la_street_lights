...
```
src/la_street_lights/bin/   # data-pipeline commands (Typer)
...
web/                        # the static site (deployed to GitHub Pages)
  index.html, app.js
  serve.py                  # local dev server (adds HTTP Range support PMTiles needs)
...
Commands run as modules (the package isn't installed; use `PYTHONPATH=src`):

```bash
# 1. Parse the assessment-roll PDFs into one CSV (~584k rows)
PYTHONPATH=src python -m la_street_lights.bin.parse_assessments

# 2. (optional) City APNs that are NOT in the assessment roll
PYTHONPATH=src python -m la_street_lights.bin.unassessed_apns
...
PYTHONPATH=src python -m la_street_lights.bin.build_map --scale log

# Run it on a single AIN range for a fast test build:
PYTHONPATH=src python -m la_street_lights.bin.build_map --scale log --ain-prefix 2004
```
...
```bash
python web/serve.py            # serves web/ at http://localhost:8765 (with Range support)
```

> Note: a plain `python -m http.server` will not work — it ignores HTTP `Range`
> requests, which PMTiles relies on. Use `web/serve.py`.
...
- The committed `*.pmtiles` are regenerated by `build_map`; they're served by
